...
test_that("test selection is not specified error",{
  expect_error(knoema.get('IMFWEO2017Apr',app.id='bHcV5UkOVyKcBw',app.secret="/0itYgLqnD0i49kmdBVSZ1qLjPU"),'argument "selection" is missing, with no default')
})
...
test_that('test empty dimension selection error',{
  expect_error(knoema.get('IMFWEO2017Apr', list(country='', subject='LP;NGDP'), app.id='bHcV5UkOVyKcBw',app.secret="/0itYgLqnD0i49kmdBVSZ1qLjPU")
...
test_that('test wrong dimension selection error',{
  expect_error(knoema.get('IMFWEO2017Apr', list(country='914;512;111', subject='L1P;N1GDP'), app.id='bHcV5UkOVyKcBw',app.secret="/0itYgLqnD0i49kmdBVSZ1qLjPU")
               ,'Selection for dimension Subject is empty')
})

context('test incorrect dataset id error')
test_that('test incorrect dataset id error',{
  expect_error(knoema.get('incorrect id', list(domedim='val1;val2'),app.id='bHcV5UkOVyKcBw',app.secret="/0itYgLqnD0i49kmdBVSZ1qLjPU"),"Requested dataset doesn't exist or you don't have access to it.")
  })

context('test incorrect app.id and app.secret error')
test_that('test incorrect app.id and app.secret error',{
  expect_error(knoema.get('IMFWEO2017Apr', list(country='914;512;111', subject='LP;NGDP'), app.id='b',app.secret="s"),"Client error: (403) Forbidden", fixed = TRUE)
})
...
test_that('test not all dims in filter error',{
  expect_error(knoema.get('bmlaaaf', list('Country'= 'Albania',
                                          ...
                                          'Guarantor'= 'Albania',
                                          'Loan type'= 'B loan',
                                          'Loan status'= 'EFFECTIVE'),app.id='bHcV5UkOVyKcBw',app.secret="/0itYgLqnD0i49kmdBVSZ1qLjPU"),
...
test_that('test incorect frequencies error',{
  expect_error(knoema.get('IMFWEO2017Apr', list(country='914;512;111', subject='LP;NGDP', frequency="A;G;R"), app.id='bHcV5UkOVyKcBw',app.secret="/0itYgLqnD0i49kmdBVSZ1qLjPU")
